...
export default function ResourceTimeline({ resource, bookings }) {
  ...
  const getPosition = (dateTimeStr) => {
    ...
    const totalPossibleMinutes = 14 * 60; // 7am to 9pm = 14 hours
    ...
    <div className="card overflow-hidden">
      <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
        ...
        <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-zinc-400">
           ...
           <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded border border-red-500 border-dashed" /> Rejected</div>
        </div>
      </div>
              ...
              <div key={day} className="flex items-center">
                ...
                <div className="flex-1 h-12 bg-zinc-100/50 rounded-lg relative border border-zinc-100 overflow-hidden">
                  ...
                  {bookings.filter(b => {
                    ...
                    // This is simplified: matching day by index for dummy data
                    return d.getDay() === (dayIdx + 1); 
                  }).map(b => (
                    <div 
                      key={b.id}
                      className={cn(
                        "absolute top-1 bottom-1 rounded-md px-2 py-1 text-[9px] font-bold overflow-hidden shadow-sm flex flex-col justify-center",
                        b.status === 'APPROVED' ? "bg-brand-500 text-white" : 
                        b.status === 'PENDING' ? "bg-amber-400 text-white" :
                        "border border-red-500 border-dashed text-red-500 bg-red-50/50"
                      )}
                      style={{ 
